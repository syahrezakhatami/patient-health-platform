...
def _vaccine(
    ...
    *,
    note: str | None = "Given at clinic",
    route: str | None = "IM",
    site: str | None = "LEFT_ARM",
) -> dict[str, object]:
    payload: dict[str, object] = {
        "patient_identity_id": patient_id,
        "category": "ADMINISTERED",
        "code": {"system": CVX, "code": "208", "display": "COVID-19 vaccine"},
    }
    if note is not None:
...
def _amend_body(*, note: str = "Corrected site", site: str = "RIGHT_ARM") -> dict[str, object]:
    return {"note_text": note, "route": "IM", "site": site}


@requires_db
async def test_immunization_lifecycle_identity_and_authorization(db_client, db_engine) -> None:
    ...
    invalid = await db_client.post(
        ...
        json={**_vaccine(patient_id), "category": "NOT_A_CATEGORY"},
    )
    assert invalid.status_code == 422
    invalid_code = await db_client.post(
        "/api/v1/clinical/immunizations",
        headers=clinician.headers(purpose="TREATMENT"),
        json={**_vaccine(patient_id), "code": {"system": "", "code": "208"}},
    )
    ...
    created = await db_client.post(
        "/api/v1/clinical/immunizations",
        headers=clinician.headers(purpose="TREATMENT"),
        json=_vaccine(patient_id, encounter_id),
    )
    assert created.status_code in {200, 201}
    body = created.json()
    assert body["status"] == "ACTIVE"
    assert body["version"] == 1
    assert body["category"] == "ADMINISTERED"
    assert body["route"] == "IM"
    immunization_id = body["id"]
    ...
    assert "COVID-19 vaccine" not in denied.text
    assert "Given at clinic" not in denied.text
    ...
    assert immunization_id in {item["id"] for item in listed.json()}
    ...
    assert amended.json()["site"] == "RIGHT_ARM"
    ...
    assert "208" not in cross.text
    assert "COVID-19 vaccine" not in cross.text
    unknown = await db_client.get(
        f"/api/v1/clinical/immunizations/{uuid4()}",
        headers=clinician.headers(purpose="TREATMENT"),
    )
    ...
    unprovisioned = mint_token(sub="nobody-immunization")
    ...
    async with db_engine.connect() as connection:
        ...
        with pytest.raises(Exception, match="immutable"):
            async with connection.begin():
                await connection.execute(
                    text("UPDATE immunizations SET code = 'changed' WHERE id = :id"),
                    {"id": immunization_id},
                )
        ...
        provenance = await connection.execute(
            text(
                ...
                WHERE id = (SELECT provenance_id FROM immunizations WHERE id = :id)
                """
            ),
            {"id": immunization_id},
        )
        assert provenance.scalar_one() == "IMMUNIZATION"
        fk = await connection.execute(
            text(
                """
                SELECT delete_rule FROM information_schema.referential_constraints
                WHERE constraint_name = 'fk_immunizations_provenance_id'
                """
            )
        )
        ...
        assert "IMMUNIZATION_CREATED" in actions
        assert "IMMUNIZATION_AMENDED" in actions
        assert "IMMUNIZATION_ENTERED_IN_ERROR" in actions
        assert all("COVID-19 vaccine" not in (row[1] or "") for row in rows)
        assert all("Given at clinic" not in (row[1] or "") for row in rows)
        assert all("208" not in (row[1] or "") for row in rows)
        later = await connection.execute(
            text(
                ...
                WHERE table_schema = 'public'
                  AND table_name IN (
                    'fhir_immunizations','fhir_procedures','care_plans'
                  )
                """
            )
        )
        ...
        present = await connection.execute(
            text(
                ...
                SELECT count(*) FROM information_schema.tables
                WHERE table_schema = 'public'
                  AND table_name IN ('immunizations','consents','allergies')
                """
            )
        )
...
@requires_db
async def test_anonymous_merged_and_encounter_immunization_binding(db_client, db_engine) -> None:
    clinician = await seed_actor(db_engine, role_code=RoleCode.CLINICIAN)
    registrar = await seed_actor(
        db_engine, role_code=RoleCode.REGISTRAR, organization_id=clinician.organization_id
    )
    ...
    cross_org = await db_client.post(
        "/api/v1/clinical/immunizations",
        headers=clinician.headers(purpose="TREATMENT"),
        json=_vaccine(patient_id, foreign.json()["id"]),
    )
    assert cross_org.status_code == 404
    unknown_enc = await db_client.post(
        "/api/v1/clinical/immunizations",
        headers=clinician.headers(purpose="TREATMENT"),
        json=_vaccine(patient_id, str(uuid4())),
    )
    ...
    blocked_cancelled = await db_client.post(
        "/api/v1/clinical/immunizations",
        headers=clinician.headers(purpose="TREATMENT"),
        json=_vaccine(patient_id, cancelled.json()["id"]),
    )
    ...
    blocked_eie_enc = await db_client.post(
        "/api/v1/clinical/immunizations",
        headers=clinician.headers(purpose="TREATMENT"),
        json=_vaccine(patient_id, erroneous.json()["id"]),
    )
            ...
            "identifiers": [
                {
                    ...
                    "identifier_type": "MRN",
                    "identifier_value": unique_mrn("W2B4"),
                }
            ],
        },
    )
    survivor = await db_client.post(
        "/api/v1/mpi/identities",
        headers=officer.headers(),
        json=_identity_payload(unique_nik(), given="Imm", family="Survivor", birth="1982-02-02"),
    )
    ...
    merged = await db_client.post(
        ...
        json={
            ...
            "reason": "Wave 2B.4 historical immunization",
            "evidence": merge_evidence("wave2b4-hist"),
        },
    )
    assert merged.status_code in {200, 201}
    fetched = await db_client.get(
        f"/api/v1/clinical/immunizations/{historical_id}",
        headers=clinician.headers(purpose="TREATMENT"),
    )
    ...
    rejected = await db_client.post(
        "/api/v1/clinical/immunizations",
        headers=clinician.headers(purpose="TREATMENT"),
        json=_vaccine(retired),
    )
    assert rejected.status_code == 409
    missing = await db_client.post(
        "/api/v1/clinical/immunizations",
        headers=clinician.headers(purpose="TREATMENT"),
        json=_vaccine(str(uuid4())),
    )
    assert missing.status_code == 404


@requires_db
async def test_immunization_concurrency_facility_and_app_dml(db_client, db_engine) -> None:
    clinician = await seed_actor(db_engine, role_code=RoleCode.CLINICIAN)
    registrar = await seed_actor(
        db_engine, role_code=RoleCode.REGISTRAR, organization_id=clinician.organization_id
    )
    ...
    created = await db_client.post(
        "/api/v1/clinical/immunizations",
        headers=clinician.headers(purpose="TREATMENT"),
        json=_vaccine(patient_id, note="concurrent amend"),
    )
    ...
    other = await db_client.post(
        "/api/v1/clinical/immunizations",
        headers=clinician.headers(purpose="TREATMENT"),
        json=_vaccine(patient_id, note="concurrent eie"),
    )
    other_id = other.json()["id"]

    async def void() -> object:
        return await db_client.post(
            f"/api/v1/clinical/immunizations/{other_id}/entered-in-error",
            headers=clinician.headers(purpose="TREATMENT"),
        )
    ...
    async def amend_race() -> object:
        return await db_client.post(
            f"/api/v1/clinical/immunizations/{race_id}/amend",
            headers=clinician.headers(purpose="TREATMENT"),
            json=_amend_body(),
        )
    ...
    async def void_race() -> object:
        return await db_client.post(
            f"/api/v1/clinical/immunizations/{race_id}/entered-in-error",
            headers=clinician.headers(purpose="TREATMENT"),
        )
    ...
    async with db_engine.connect() as connection:
        row = await connection.execute(
            text("SELECT status FROM immunizations WHERE id = :id"),
            {"id": race_id},
        )
    ...
    out_of_scope = new_id()
    async with db_engine.begin() as connection:
        for facility_id, code in ((in_scope, "IIN"), (out_of_scope, "IOUT")):
            await connection.execute(
    ...
    async with db_engine.connect() as connection:
        provenance = await connection.execute(
            text("SELECT provenance_id FROM immunizations WHERE id = :id"),
            {"id": immunization_id},
        )
        provenance_id = provenance.scalar_one()
    async with db_engine.connect() as connection:
        with pytest.raises(Exception, match="foreign key|fk_immunizations_provenance"):
            async with connection.begin():
                await connection.execute(
                    text(
                        """
                        ...
                        FROM immunizations WHERE id = :id
                        """
                    ),
                    {"id": immunization_id, "bad": uuid4()},
                )
        with pytest.raises(Exception, match="insert-only|foreign key|fk_immunizations_provenance"):
            async with connection.begin():
                await connection.execute(
                    text("DELETE FROM clinical_provenances WHERE id = :id"),
                    {"id": provenance_id},
                )
        with pytest.raises(Exception, match="invalid immunization status transition"):
            async with connection.begin():
                await connection.execute(
                    text("UPDATE immunizations SET status = 'ACTIVE' WHERE id = :id"),
                    {"id": immunization_id},
                )
